...
before(function(done){
  ...
  ckan_govdata_rostock = new require('../models/ckan_govdata_rostock.js')( koop );
  done();
});

describe('ckan_govdata_rostock Model', function(){

    afterEach(function(done){
      done();
    });

    describe('ckan_govdata_rostock model methods', function() {
      before(function(done ){
        done();
      });
      it('toGeoJSON should err when given no data', function(done) {
        ckan_govdata_rostock.toGeojson([], 'location', function(err, geojson){
          should.exist(err);
      ...
      it('toGeoJSON should return geojson', function(done) {
        ckan_govdata_rostock.toGeojson(data, 'location', function(err, geojson){
      ...
      it('getResource should return geojson', function(done) {
        ckan_govdata_rostock.getResource('https://data.seattle.gov', '2tje-83f6', {}, function(err, geojson){
          should.not.exist(err);
          should.exist( geojson );
